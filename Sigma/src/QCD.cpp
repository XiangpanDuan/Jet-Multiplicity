...
double QCD::f2loop(const double t, const double b0, const double b1){
  ...
  return alphaS_mZ2*std::pow(b0,3)*std::pow(t,2)-b0*b0*t+b1*std::log(t);
}
//f'(t)
double QCD::df2loop(const double t, const double b0, const double b1){
  double alphaS_mZ2=0.118;
  return 2.*alphaS_mZ2*std::pow(b0,3)*t-b0*b0+b1/t;
}
...
double QCD::LambdaQCD(const unsigned int nloop){
  ...
  if(nloop==1){_lambdaQCD=_Z0->Mass()/std::pow(std::exp(1./(b0*alphaS_mZ2)),0.5);}  //_lambdaQCD=0.245748 for _nf=3
  ...
  if(nloop==2){
    ...
    do{
      ...
      iter++;
      if(iter>itermax){
        std::cerr << "The maximum iteration is reached for LambdaQCD calculation in QCD.cpp." << std::endl;
        std::exit(EXIT_FAILURE);
      }
    }while(std::abs(delta)>errtol);
    _lambdaQCD=std::sqrt(_Z0->Mass()*_Z0->Mass()/std::exp(t));  //_lambdaQCD=0.73764396 for _nf=3
  }
  return _lambdaQCD;
}
